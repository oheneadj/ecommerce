...
--}}
<nav aria-label="{{ __('Breadcrumb') }}" class="mb-4 text-sm text-zinc-500 dark:text-zinc-400">
    <ol class="flex flex-wrap items-center gap-1.5">
        @foreach ($items as $index => $item)
            <li class="flex items-center gap-1.5">
                ...
                @else
                    <span class="text-zinc-700 dark:text-zinc-300" @if($index === count($items) - 1) aria-current="page" @endif>{{ $item['label'] }}</span>
                @endif
            </li>
        @endforeach
    </ol>
