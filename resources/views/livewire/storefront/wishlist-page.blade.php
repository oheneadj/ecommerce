<div class="space-y-6">
    ...
    @if ($this->items->isEmpty())
        <x-card>
            <p class="text-sm text-zinc-500">{{ __('Your wishlist is empty.') }}</p>
        </x-card>
    @else
        <x-card>
            <div class="divide-y divide-zinc-200">
                @foreach ($this->items as $wishlistItem)
                    ...
                    <div wire:key="wishlist-item-{{ $wishlistItem->id }}" class="flex flex-wrap items-center gap-4 py-4">
                        <a href="{{ route('products.show', $product) }}" wire:navigate class="-m-2 flex min-w-0 flex-1 basis-40 items-center gap-4 rounded-lg p-2 transition-colors hover:bg-zinc-50">
