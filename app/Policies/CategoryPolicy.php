<?php

declare(strict_types=1);

namespace App\Policies;

use App\Enums\UserRole;
use App\Models\Category;
use App\Models\User;

/**
 * Authorization rules for Category: catalog staff can view/manage
 * categories, but only Admins/Super Admins can delete or restore them.
 */
class CategoryPolicy
{
    /**
     * Any catalog-facing role can view the category list.
     */
    public function viewAny(User $user): bool
    {
        return $user->hasAnyRole([UserRole::SuperAdmin->value, UserRole::Admin->value, UserRole::StoreKeeper->value]);
    }

    /**
     * Same rule as viewAny.
     */
    public function view(User $user, Category $category): bool
    {
        return $this->viewAny($user);
    }

    /**
     * Same rule as viewAny.
     */
    public function create(User $user): bool
    {
        return $this->viewAny($user);
    }

    /**
     * Same rule as viewAny.
     */
    public function update(User $user, Category $category): bool
    {
        return $this->viewAny($user);
    }

    /**
     * Admins/Super Admins only.
     */
    public function delete(User $user, Category $category): bool
    {
        return $user->hasAnyRole([UserRole::SuperAdmin->value, UserRole::Admin->value]);
    }

    /**
     * Same rule as delete.
     */
    public function restore(User $user, Category $category): bool
    {
        return $this->delete($user, $category);
    }

    /**
     * Super Admin only.
     */
    public function forceDelete(User $user, Category $category): bool
    {
        return $user->hasRole(UserRole::SuperAdmin->value);
    }
}
