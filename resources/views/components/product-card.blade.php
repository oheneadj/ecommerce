...
<a href="{{ route('products.show', $product) }}" wire:navigate class="group block overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
    <div class="flex aspect-square items-center justify-center bg-zinc-100 dark:bg-zinc-800">
        @if ($image)
            <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($image->path) }}" alt="{{ $product->name }}" class="h-full w-full object-cover transition group-hover:scale-105">
        @else
